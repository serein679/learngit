package com.shxy.sudatqd.mapper;
import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.shxy.models.dbBean.*;
import com.shxy.models.views.archiveItem;
import com.shxy.models.views.componentItem;
import com.shxy.models.views.fiberItem;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import java.util.List;

@Mapper
public interface fiberMapper extends BaseMapper<fiber> {
    List<fiberItem> selectFiberItems(@Param("fiberType") String fiberType,
                                     @Param("characteristic") String characteristic,
                                     @Param("userName") String userName,
                                     @Param("state") Integer state,
                                     @Param("orderColumn") String orderColumn,
                                     @Param("isAsc") boolean isAsc);

    fiberItem selectOneFiberItem(@Param("fiberId") Integer fiberId);
}
