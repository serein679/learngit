<template>
  <div style="margin-top: 8px">
    <div>
          ...
          <el-row style="margin-top: 15px; ">
            ...
            <el-col :span="13" style="text-align: right">
              <el-button type="success" icon="el-icon-plus" @click="addPaper"
                style="background-color:peru;border: 0;font-family: STKaiti;font-size: 18px;margin-left: 22px;">新增纤维原料
              </el-button>
            </el-col>
          </el-row>
      ...
      <el-table v-if="isShow" ref="table" :data="fiberList" @filter-change="_filterChange" class="table" empty-text="暂无资源"
        ...
        <el-table-column label="纤维原料" prop="fiberType" width="200px">
          <template slot-scope="scope" v-for="item in fiberList">
            <div style="font-size: 18px;">
              <el-link @click="select(scope.row.fiberId)">
                <div style="font-size: 18px;">{{ scope.row.fiberType }}</div>
              </el-link>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="纤维老化情况" prop="characteristic" width="350px">
        </el-table-column>
        <el-table-column label="原料状态" prop="state" width="230px">
          <template slot-scope="props">
            {{ typeName1[props.row.state] }}
          </template>
        </el-table-column>
        <el-table-column label="上传人" prop="creator" width="230px">
        </el-table-column>
        ...
        <el-table-column label="管理" width="200px" fixed="right" prop="state">
          <template slot-scope="props" style="font-size: 18px;">
            <el-button @click="updateClick(props.row.fiberId)" type="text" size="big" style="font-size: 18px;"
              v-if="props.row.state != 1">
              编辑</el-button>
            <el-button @click="deleClick(props.row.fiberId)" type="text" size="big" style="font-size: 18px;"
              v-if="props.row.state != 1">删除</el-button>
            <el-button @click="deleClick1(props.row.fiberId)" type="text" size="big" style="font-size: 18px;"
      ...
      <pagination style="width:100%; margin: auto;font-family: STKaiti;" v-show="total1 > 0" :page.sync="currentPage"
        ...
        :total="Number(total1)" @pagination="PageAllFiber"><!--分页-->
      </pagination><!--分页-->
      <!--资源详情管理编辑框-->
    </div>
  </div>
...
import { PageAllFiber, deleFiber} from '@/api/fiber'
...
export default {
  ...
  methods: {
    ...
    PageAllFiber() {
      ...
      var isAsc = false
      var fiberType = ""//排序
      var characteristic = ""//排序
      var orderColumn = "fiberType"
      var creator = ""//排序
      var state = ""//排序      
      PageAllFiber({ pageSize, isAsc , pageNum, fiberType , characteristic,orderColumn,creator,state}).then((response) => {
        if (response) {
          this.total1 = response.data.total;
          this.reload();//局部刷新
          console.log(pageNum + "/" + pageSize + "/" + this.total);
          this.fiberList = response.data.list;
        } else {
    ...
    // 添加主页项目
    addPaper() {
      this.$router.push('/management/addIntro');
    },
    //删除
    deleClick(fiberId) {
      console.log(fiberId);//已获取
      this.$confirm('您确定要删除该原料吗?', '提示', {
        ...
        var id = fiberId
        var status = 1
        deleFiber({id,status}).then(response => {
          if (response.message == "纤维状态已更新！") {
            this.$message({
              type: 'success',
              message: "纤维状态已更新！"
            })
            ...
            this.PageAllFiber(this.order)
            // this.PageAllFiber(order = 0)
