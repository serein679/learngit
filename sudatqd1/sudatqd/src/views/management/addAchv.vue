  ...
  <div> <!--添加专业对话框-->
    <div style="padding: 10px;">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/management/addAchv' }">新增大事记</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div slot="title">
      <!-- <span style="font-family: '楷体'; font-size: 20px;">新增纸张</span> -->
    </div>
    <span style="font-family: '楷体'; font-size: 26px;">新增大事记</span>
    <el-tabs class="el-tab-div" style="font-family: 楷体;font-size: 26px;">
      <el-form :model="addHis" ref="addHis" style="font-size: 32px !important" :rules="rules">
        <el-form-item label="事记名称" :label-width="formLabelWidth" prop="eventName">
          <el-input v-model="addHis.eventName"></el-input>
        </el-form-item>
        <el-form-item label="事记详情" :label-width="formLabelWidth" prop="eventDescription">
          <WangEditor @sendWangEditor="changeOutline" :editorData="editor"></WangEditor>
          <!-- <el-input v-model="addHis.eventDescription" type="textarea" :rows="5" prop="eventDescription"></el-input> -->
        </el-form-item>
        <el-form-item label="大事记标题" :label-width="formLabelWidth" style="width: 630px;">
          ...
          <img :src="addHis.eventPic" width="40%" style="float: center;margin-top: 10px;">
        </el-form-item>

        <el-form-item label="事记出现时间" :label-width="formLabelWidth" style="margin-left: 1px;width: 300px;"
          prop="eventDate" @change="chooseTimeRange">
          <!-- <el-input v-model="oneproject.projectStartTime"></el-input> -->
          <el-date-picker v-model="addHis.eventDate" type="date" placeholder="选择日期" align="right"
...
import { getHistoricalEvents, getOneHistoricalEvent, editEvent, PageAllHis, addEvent } from '@/api/his'
...
export default {
  ...
  data() {
    return {
      rules: {
        ...
        eventDescription: [
          { required: true, message: '请输入机事记详情', trigger: 'blur' },
        ...
        eventDate: [
          { required: true, message: '请输入大事记出现时间', trigger: 'blur' },
        ],
      },
      activeName1: '1',
      activeName: '1',
  ...
  methods: {
    ...
    changeOutline(val) {
      this.addHis.eventDescription = val;
    },
    ...
    // 编辑按钮点击事件
    addEvent() {
      console.log(this.addHis); // 打印 addresource 对象
      if (this.addHis.eventName == "") {
        this.$message({
          type: 'warning',
          message: '事记名称为空，请填写！'
        })
        ...
        this.$refs.addHis.validate(valid => {
          if (valid) {
            this.$confirm('您确定要添加该事记吗?', '提示', {
              ...
              formData.append('eventName', this.addHis.eventName);
              // formData.append('institutionLocation', this.addInstitution.institutionLocation);
              formData.append('eventDate', new Date(this.addHis.eventDate));
              formData.append('eventDescription', this.addHis.eventDescription);
              formData.append('eventPic', this.addHis.eventPic);
