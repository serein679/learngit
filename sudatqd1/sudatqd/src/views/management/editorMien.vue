<template>
  <div slot="title">
    ...
    <span style="font-family: 'STKaiti'; font-size: 26px;">编辑机构</span>
    <el-tabs class="el-tab-div" style="font-family: STKaiti;font-size: 26px;">
      <el-form :model="oneInstitution" ref="oneInstitution" style="font-size: 32px !important" >
        <el-form-item label="机构名称" :label-width="formLabelWidth" prop="institutionName">
          <el-input v-model="oneInstitution.institutionItem.institutionName"></el-input>
        </el-form-item>
        <el-form-item label="机构简介" :label-width="formLabelWidth" prop="institutionDescription">
          <el-input v-model="oneInstitution.institutionItem.institutionDescription" type="textarea" :rows="5"
        ...
        <el-form-item label="相关档案" :label-width="formLabelWidth">
          <template slot-scope="props">
            <el-select v-model="oneInstitution.archiveList" :props="{ key: 'archiveId', label: 'archiveName' }" multiple
      ...
      <el-dialog title="地图定位" :visible.sync="dialogVisible" @close="close" width="70%" style="margin-left: 10%;">
        <el-form ref="form" :model="oneInstitution" label-width="100px">
          ...
          <el-form-item label="经度：">
            <el-input v-model="oneInstitution.institutionItem.longitude" ></el-input>
          </el-form-item>
          <el-form-item label="维度：">
            <el-input v-model="oneInstitution.institutionItem.latitude" ></el-input>
          </el-form-item>
        </el-form>
        ...
        <el-form-item label="机构出现时间" :label-width="formLabelWidth" style="margin-left: 1px;width: 300px;"
          ...
          <el-date-picker v-model="oneInstitution.institutionItem.institutionDate" type="date" placeholder="选择日期" align="right"
  ...
  close(){
      ...
      initMap() {
        try {
          ...
          var point = new BMap.Point(
            this.oneInstitution.longitude || 116.41338729034514,
            this.oneInstitution.latitude || 39.910923647957596
          )
      ...
      getAddrByPoint(point) {
        ...
        geco.getLocation(point, function (res) {
          ...
          that.oneInstitution.longitude = res.point.lng
          that.oneInstitution.latitude = res.point.lat
          console.log(that.oneInstitution.longitude)
          console.log(that.oneInstitution.latitude)
      ...
      handleSelect(item) {
        ...
        console.log(item);
        this.oneInstitution.longitude = item.point.lng
        this.oneInstitution.latitude = item.point.lat
    ...
    getOneIn(institutionId) {
      getOneIn(institutionId).then((response) => {
        this.oneInstitution = response.data;
        this.paperList1 = this.oneInstitution.paperList1;
        this.fileName = this.oneInstitution.institutionPicture;
        this.selectedOptions = TextToCode[this.oneInstitution.institutionItem.institutionLocation.split('/')[0]][this.oneInstitution.institutionItem.institutionLocation.split('/')[1]][this.oneInstitution.institutionItem.institutionLocation.split('/')[2]].code;
        ...
        this.$refs.oneInstitution.validate(valid => {
          if (valid) {
            this.$confirm('您确定要修改该机构吗?', '提示', {
              ...
              formData.append('longitude', this.oneInstitution.longitude);
              formData.append('latitude', this.oneInstitution.latitude);
