<template>
    <div style="margin-top: 8px">
      <div>
        ...
        <el-col :span="6" class="filter"  style="text-align: left;">
          <span style="font-family: STKaiti;font-size: 18px;">敏感词类型：
        ...
        <el-table v-if="isShow" ref="table" :data="hisList" @filter-change="_filterChange" class="table" empty-text="暂无资源"
          ...
          <el-table-column label="序号" width="170px" type="index" :index="indexMethods"
            style="text-align: center;"></el-table-column>
          <el-table-column label="敏感词" show-overflow-tooltip prop="wordName" width="220px">
            <template slot-scope="scope" v-for="item in hisList">
              <div style="font-size: 18px;cursor: pointer;">
                  <div style="font-size: 18px;">{{ scope.row.wordName
                  }}</div>
              </div>
            </template>
          </el-table-column>
  
          <el-table-column label="敏感词类型" prop="typeName" width="320px">
          </el-table-column>
          ...
          <el-table-column label="敏感词状态" prop="state" width="180px">
        ...
        <pagination style="width:100%; margin: auto;font-family: STKaiti;" v-show="total1 > 0" :page.sync="currentPage"
          ...
          :total="Number(total1)" @pagination="getWordPageList"><!--分页-->
        </pagination><!--分页-->
  ...
  import { getWordPageList } from '@/api/word'
  ...
  export default {
    ...
    data() {
      return {
        ...
        pageSizes: [ 5, 10, 20, 30, 50],// 个数选择器（可修改）
    ...
    methods: {
      ...
      getWordPageList() {
        ...
        var isAsc = false 
        var orderColumn = "wordId"//排序
        var state = ''//排序      
        var wordName = ''//排序    
        var type = ''//排序  
        var creator = ''//排序
        getWordPageList({ pageSize, creator,wordName, type, pageNum, orderColumn,isAsc,state }).then((response) => {
          if (response) {
