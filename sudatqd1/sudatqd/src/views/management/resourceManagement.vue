  ...
  <div style="margin-top: 8px">
    <div>
      ...
      <el-table v-if="isShow" ref="table" :data="hisList" @filter-change="_filterChange" class="table" empty-text="暂无资源"
        ...
        <el-table-column label="角色名称" show-overflow-tooltip prop="roleName" width="180px">
          <template slot-scope="scope" v-for="item in hisList">
            <div style="font-size: 18px;cursor: pointer;">
                <div style="font-size: 18px;">{{ scope.row.roleName
                }}</div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="角色状态" prop="state" width="180px">
...
import { getRolePageList,changeRoleStatus } from '@/api/role'
...
export default {
  ...
  methods: {
    ...
    getRolePageList() {
      ...
      var isAsc = false 
      var orderColumn = "roleId"//排序
      var state = ''//排序      
      var roleName = ''//排序    
      var description = ''//排序  
      var creator = ''//排序
      getRolePageList({ pageSize, creator,roleName, description, pageNum, orderColumn,isAsc,state }).then((response) => {
        if (response) {
